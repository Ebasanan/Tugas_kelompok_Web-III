...
/***	Mencari kode otomatis untuk kategori 	***/
	...
	// jika didalam tabel_kategori belum ada data maka kita akan membuat kode awal untuk kategori adalah ST00
	...
	// namun jika sudah ada data kita akan mencari data terakhir dan menyeleksi kode terkahir yang terletak pada kolom_kode_kategori
	else {

		while ($row1 = mysqli_fetch_array($sql1)) {

			$kode_terakhir = $row1["kolom_kode_kategori"];
	...
	$kodeKategori 	=	kode_otomatis($kode_terakhir, 'KT', 2);

/*** Proses penambahan data kategori ***/
	...
	// Jika tombol simpan yang bernama submitKategori di klik
		...
		if (count($pesan)>=1) {
			...
			if ($sql3==true) {
				...
				echo 	"<script>";
				echo 	"
							if (confirm('Berhasil menambahkan data kategori')) {
...
<div class="card">
	...
	<div class="card-body">
		...
		<h3 class="card-subtitle">Tambah Kategori</h3><!-- .card .card-body .card-subtitle -->

		<form method="post" class="form" name="formTambahKategori">
					
			<label>Kode Kategori</label><!-- .card .form label -->
			<input type="text" disabled value="<?php echo $kodeKategori; ?>"><!-- .card .form input[type=text] -->
